...
// ---------- Date helpers (avoid brittle string comparisons) ----------
function toMs(iso) {
...
// ---------- Payments -> Orders (more reliable for "paid") ----------
async function listPaymentsAll({ beginTime, endTime }) {
  // ListPayments is eventually consistent; dashboard is refreshed on demand.
  // Ref: GET /v2/payments (max 100 per page)
  ...
  while (true) {
    ...
    const result = await callSquare(`/v2/payments?${params.toString()}`, "GET");
    if (result.statusCode >= 400 || result.body.errors) {
      console.error("Square payments error:", result.body);
      throw new Error(
        result.body.errors?.[0]?.detail || "Error loading payments from Square"
      );
    }

    const payments = result.body.payments || [];
    for (const p of payments) out.push(p);
...
async function batchRetrieveOrders(orderIds) {
  ...
  for (let i = 0; i < orderIds.length; i += chunkSize) {
    ...
    const result = await callSquare("/v2/orders/batch-retrieve", "POST", {
      location_id: LOCATION_ID,
      order_ids: chunk,
    });

    if (result.statusCode >= 400 || result.body.errors) {
      console.error("Square batch retrieve error:", result.body);
      throw new Error(
        result.body.errors?.[0]?.detail || "Error retrieving orders from Square"
      );
    }
...
exports.handler = async (event) => {
  ...
  try {
    // 1) Bring only PAID payments, then pull their related orders.
    //    Square explicitly documents this flow: retrieve payments -> BatchRetrieveOrders by Payment.order_id.
    //    https://developer.squareup.com/docs/orders-api/manage-orders/retrieve-orders
    const payments = await listPaymentsAll({
      beginTime: "2025-12-01T00:00:00-05:00",
    ...
    const orderIds = Array.from(
      new Set(
        payments
          .filter((p) => p && p.status === "COMPLETED" && p.order_id)
          .map((p) => p.order_id)
      )
    );

    const orders = await batchRetrieveOrders(orderIds);
    ...
    for (const order of orders) {
      ...
      if (order.created_at && isBefore(order.created_at, GO_LIVE_AT)) continue;
